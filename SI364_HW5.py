...
app.config["SQLALCHEMY_DATABASE_URI"] = "postgresql://localhost/margauxkHW5"
...
app.config['MAIL_SENDER'] = 'Admin <margauxk364@gmail.com>' # TODO fill in email
app.config['ADMIN'] = 'margauxk364@gmail.com'
...
class TweetForm(FlaskForm):
    ...
    user_email = StringField("Enter your email here: ", validators=[Required()])
...
def get_or_create_user(db_session, username, user_email):
    user = db_session.query(User).filter_by(twitter_username=username, user_email = user_email).first()
    if user:
        return user
    else:
        user = User(twitter_username=username, user_email = user_email)
        db_session.add(user)
        db_session.commit()
        return user
...
def index():
    ...
    if form.validate_on_submit():
        if db.session.query(Tweet).filter_by(text=form.text.data, user_id= (get_or_create_user(db.session, form.username.data, form.user_email.data).id)).first():
            flash("You've already saved that tweet by this user!")
        else:
            get_or_create_tweet(db.session, form.text.data, form.username.data, form.user_email.data)
